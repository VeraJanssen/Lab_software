...
filename  = '2K_dev16_cont5-8_mol_gate_trace'
...
ramp(vi,'Vb',Vb_fixed,bias_step_init,bias_time)
...
print 'Starting I vs Vgate sweet at Vb=const B = %s T' %B
...
append = '_Vb%smV'%Vb_fixed+'_Vg%sV'%Vg_start+'_B%sT'%B 
...
plot2d = qt.Plot2D(data, coorddim=0, valdim=2 ,traceofs=0)
...
for Vg in Vg_vec:
    ...
    data.add_data_point(Vg,Vb_fixed,I)
#data.new_block()    
#spyview_process(data,Vb_start,Vb_stop,Vg)
spyview_process(data,Vb_fixed,Vb_fixed,Vg)
