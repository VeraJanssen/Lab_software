# time_temp.py, modified by Rocco Gaudenzi (r.gaudenzi@tudelft.nl)
...
t  =  2        # minutes
dt = .50       # seconds
N_traces = 100 # total time --> t*N_traces; every t temperature value is taken 
...
data.add_coordinate('T (mK)')  #parameter to step
...
for N in N_vec:
    ...
    while (timer < t_sec):
        ...
        data.add_data_point(T_mc_st,timer,I)
        qt.msleep(dt-.040)
    T_mc_end = T_mc()
    print 'T_mc_end = %i mK'%T_mc_end 
    #T = mean([T_mc_start,T_mc_end]) 
    spyview_process(data,0,t_sec,T_mc_st)
    data.new_block()
